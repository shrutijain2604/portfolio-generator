  ...
  return (
    <div className="relative min-h-dvh overflow-hidden" style={{ backgroundColor: "#030208" }}>
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-16 sm:px-10 sm:py-20 lg:px-16">
        {/* Hero cover card */}
        <HoloCard tiltDegrees={6} className="mx-auto max-w-lg">
          <RarityBorder color={RARITY_COLORS[0]}>
            <div className="flex flex-col items-center px-8 py-10 text-center lg:px-12 lg:py-14">
              {photoUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={photoUrl} alt="" className="h-20 w-20 rounded-full border-2 object-cover lg:h-24 lg:w-24" style={{ borderColor: RARITY_COLORS[0] }} />
              ) : (
                <div
                  className="flex h-20 w-20 items-center justify-center rounded-full border-2 text-xl font-bold text-white lg:h-24 lg:w-24 lg:text-2xl"
                  style={{ background: `linear-gradient(135deg, ${RARITY_COLORS[1]}, ${RARITY_COLORS[3]})`, borderColor: RARITY_COLORS[0] }}
                >
                  {initials(name)}
                </div>
              )}
              <h1 className="mt-5 break-words text-3xl font-bold tracking-tight lg:text-4xl" style={{ color: INK }}>
                {name || "Your Name"}
              </h1>
              <p
                className="mt-1.5 inline-block rounded-full px-3 py-1 text-xs font-bold uppercase tracking-[0.2em]"
                style={{ backgroundColor: tint(RARITY_COLORS[0], 20), color: RARITY_COLORS[0] }}
              >
                {role || "Your Role"}
              </p>
              {bio && (
                <p className="mt-4 max-w-sm break-words whitespace-pre-line text-[13.5px] leading-relaxed" style={{ color: INK_SOFT }}>
                  {bio}
                </p>
              )}
              {statLine && (
        ...
        {/* The deck */}
        {entryCards.length > 0 && <div className="mt-14 columns-1 sm:columns-2 sm:gap-6 lg:columns-3">{entryCards}</div>}
