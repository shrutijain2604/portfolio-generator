...
// A translucent tint of a dotColor() result, for chip/pill backgrounds.
// dotColor() returns either a hex string or an hsl() function depending on
// the label — appending an alpha hex suffix (the usual `${color}1f` trick)
// only works for the hex case, so this uses color-mix() instead, which
// works uniformly regardless of which format dotColor() returned.
export function tint(color, percent = 12) {
  return `color-mix(in srgb, ${color} ${percent}%, transparent)`;
}

// A darkened version of a color, for single-hue gradients (accent fading
// into its own shadow) that stay legible under overlaid light text
// regardless of the source color or how light/dark the rest of the theme
// is — unlike mixing two arbitrary palette colors, which can land on two
// colors that are both too light (or both too dark) for the overlay to
// read against.
export function shade(color, percent = 45) {
  return `color-mix(in srgb, ${color}, black ${percent}%)`;
}

// "#rrggbb" -> "r, g, b", for spots that need the channel-list form (e.g.
// CursorGlow's colorRgb prop, built into an rgba() string internally) fed
// from a palette color chosen at render time rather than hardcoded.
